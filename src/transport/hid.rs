...
use crate::{LEDGER_CHANNEL, LEDGER_PACKET_SIZE, LEDGER_USAGE_PAGE, LEDGER_VENDOR_ID};
...
impl HidTransport {
    pub fn new() -> Result<HidTransport, Error> {
        let api = HidApi::new().map_err(|e| Error::HidError(e))?;
        let path = get_device_path(&api)?;
        let device = api.open_path(&path).map_err(|e| Error::HidError(e))?;
        Ok(HidTransport {
...
impl Transport for HidTransport {
    ...
    async fn exchange(&self, command: &[u8]) -> Result<Vec<u8>, Self::Error> {
        ...
        for (i, chunk) in payload.chunks(LEDGER_PACKET_SIZE - 5).enumerate() {
            ...
            let size = guard.write(&buffer).map_err(|e| Error::HidError(e))?;
            if size < buffer.len() {
        ...
        let length = reader
            .read_u16::<BigEndian>()
            .map_err(|_| Error::Deserialization)? as usize;
...
fn read_channel_tag_idx(reader: &mut Cursor<&Vec<u8>>) -> Result<(u16, u8, u16), Error> {
    let channel = reader
        .read_u16::<BigEndian>()
        .map_err(|_| Error::Deserialization)?;
    let tag = reader.read_u8().map_err(|_| Error::Deserialization)?;
    let idx = reader
        .read_u16::<BigEndian>()
        .map_err(|_| Error::Deserialization)?;
    Ok((channel, tag, idx))
...
#[cfg(target_os = "linux")]
pub fn get_device_path(api: &HidApi) -> Result<&CStr, Error> {
    for device in api.device_list() {
        if device.vendor_id() == LEDGER_VENDOR_ID {
            let usage = if device.usage_page() != 0 {
                device.usage_page()
            } else {
                return Err(Error::NoUsagePath);
            };
